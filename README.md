...
- **Snapshot / restore.** Every `SandboxedWorkspaceExtBase` exposes a uniform `snapshot(tag)` / `restore(tag)` API for iterative agent workflows (try → test → roll back). `VFSWorkspaceBase` ships a real deep-copy implementation; backends without a snapshot primitive raise `NotImplementedError` so callers can degrade gracefully. On the `agentfs` baseline, snapshot/restore is **9.1× faster** than the close + reprovision + reseed path for the iterative rollback workflow. See [`docs/SNAPSHOT.md`](docs/SNAPSHOT.md) for the open-source survey (E2B / Firecracker / gVisor / containerd / k8s), the design closure, and the benchmark numbers; [`docs/SESSION_LOG.md`](docs/SESSION_LOG.md) records the investigation / decision / verification trail.
- **Real tests, no mocking.** The guest-agent wire protocol is exercised end-to-end against the exact handler source string that ships into the microVM (run on a Unix socket in CI since AF_VSOCK is unavailable). The Firecracker REST API client is driven against a real Unix-socket HTTP server. Snapshot/restore correctness is verified by a real `diff -r` subprocess, not by equality assertions on mock state. 160+ tests, 0 mocks.
