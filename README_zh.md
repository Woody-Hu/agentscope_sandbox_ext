...
为 [agentScope](https://github.com/agentscope-ai/agentscope) 框架提供的扩展沙箱后端 —— **Firecracker microVM**、**gVisor (runsc)**、**Kata Containers** 与 **Sysbox**。

本包在不修改任何 agentscope 原生代码的前提下，新增四种 sandboxed-workspace 后端。所有后端均通过继承 agentscope 自身（且文档明确支持子类化）的抽象基类组合而成：
...
- **池化优化。** `SandboxPool` 维持 `min_warm` 个热沙箱，超过 `idle_ttl` 的空闲实例会被驱逐，热池上限为 `max_size`。每个 manager 可在自身缓存前串联一个 pool。可调 `acquire_strategy`（`fifo`/`lifo`）、可选 `health_check` 获取时探针、`max_concurrent_provisions` 信号量以防止惊群启动。详见 [`docs/PERFORMANCE_zh.md`](docs/PERFORMANCE_zh.md)。
- **真实测试，无 mock。** guest-agent 线协议使用打包进 microVM 的同一份 handler 源码做端到端验证（CI 中由于 AF_VSOCK 不可用，改用 Unix socket）；Firecracker REST API 客户端驱动的是真实的 Unix-socket HTTP 服务端。共 120+ 个测试，0 处 mock。
