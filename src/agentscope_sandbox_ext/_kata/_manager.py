# -*- coding: utf-8 -*-
"""KataWorkspaceManager — lifecycle manager for :class:`KataWorkspace`.

Thin specialisation of
:class:`DockerRuntimeWorkspaceManagerBase` that constructs
:class:`KataWorkspace` instances and tags cache entries with the
Kata backend discriminator.
"""
...
from .._kata._constants import KATA_DEFAULT_RUNTIME_NAME
from .._kata._workspace import KataWorkspace


class KataWorkspaceManager(DockerRuntimeWorkspaceManagerBase):
    """Manages :class:`KataWorkspace` instances with TTL-based
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the Kata workspace manager.
        ...
        Args:
            ...
            :data:`KATA_DEFAULT_RUNTIME_NAME`):
                Docker runtime name.  Override only if your daemon
                registers Kata under a different key.
        """
        super().__init__(
            basedir,
            isolation=isolation,
    ...
    def _make_workspace(
        ...
        *,
        ...
        """Construct a :class:`KataWorkspace` from manager config."""
        host_workdir = self._workdir_for(user_id=user_id, agent_id=agent_id)
        return KataWorkspace(
