# -*- coding: utf-8 -*-
"""SysboxWorkspaceManager — lifecycle manager for :class:`SysboxWorkspace`.

Thin specialisation of
:class:`DockerRuntimeWorkspaceManagerBase` that constructs
:class:`SysboxWorkspace` instances and tags cache entries with the
Sysbox backend discriminator.
"""
...
from .._sysbox._constants import SYSBOX_DEFAULT_RUNTIME_NAME
from .._sysbox._workspace import SysboxWorkspace


class SysboxWorkspaceManager(DockerRuntimeWorkspaceManagerBase):
    """Manages :class:`SysboxWorkspace` instances with TTL-based
    ...
    def __init__(
        ...
        *,
        ...
        """Initialize the Sysbox workspace manager.
    ...
    def _make_workspace(
        ...
        *,
        ...
        """Construct a :class:`SysboxWorkspace` from manager config."""
        host_workdir = self._workdir_for(user_id=user_id, agent_id=agent_id)
        return SysboxWorkspace(
